...
fileprivate func mutate<T>(_ arg: inout T, _ body: (inout T) -> Void) {
    body(&arg)
}
...
struct ZoomableScrollView<Content: View>: View {
    ...
    @State private var onDisappearHandler = PassthroughSubject<Void, Never>()
    
    var body: some View {
        ZoomableScrollViewImpl(content: content, doubleTap: doubleTap.eraseToAnyPublisher(), onDisappearHandler: onDisappearHandler.eraseToAnyPublisher())
...
fileprivate struct ZoomableScrollViewImpl<Content: View>: UIViewControllerRepresentable {
    ...
    let onDisappearHandler: AnyPublisher<Void, Never>
    
    func makeUIViewController(context: Context) -> ViewController {
        return ViewController(coordinator: context.coordinator, doubleTap: doubleTap, onDisappear: onDisappearHandler)
    }
    ...
    func updateUIViewController(_ viewController: ViewController, context: Context) {
        viewController.update(content: self.content, doubleTap: doubleTap, onDisappear: onDisappearHandler)
    }
    ...
    class ViewController: UIViewController, UIScrollViewDelegate {
        ...
        var doubleTapCancellable: AnyCancellable?
        var onDisappearCancellable: AnyCancellable?
        ...
        init(coordinator: Coordinator, doubleTap: AnyPublisher<Void, Never>, onDisappear: AnyPublisher<Void, Never>) {
            ...
            doubleTapCancellable = doubleTap.sink { [unowned self] _ in self.handleDoubleTap() }
            onDisappearCancellable = onDisappear.sink { [unowned self] _ in self.handleOnDisappear() }
        }
        
        func update(content: Content, doubleTap: AnyPublisher<Void, Never>, onDisappear: AnyPublisher<Void, Never>) {
            coordinator.hostingController.rootView = content
            scrollView.setNeedsUpdateConstraints()
            doubleTapCancellable = doubleTap.sink { [unowned self] _ in handleDoubleTap() }
            onDisappearCancellable = onDisappear.sink { [unowned self] _ in self.handleOnDisappear() }
        }
        ...
        func handleDoubleTap() {
            scrollView.setZoomScale(scrollView.zoomScale > 1 ? scrollView.minimumZoomScale : 2, animated: true)
        }
